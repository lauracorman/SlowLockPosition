...
import scipy
import threading
from CamerasClass import LumeneraCam
...
#        Motor locks
        ...
        self.atomsX0 = 550
        self.atomsY0 = 440
        self.maxNumberOfSteps = 500
        if not os.path.isfile('targets.txt'): 
            self.targetX = self.atomsX0
            self.targetY = self.atomsY0     
            print 'No target file found, loading very old values'
        else:
            targets = np.genfromtxt('targets.txt')
            self.targetX = targets[0]
            self.targetY = targets[1]
        self.atomsX0 = self.targetX
        self.atomsY0 = self.targetY
...
#==============================================================================
    ...
    def takePictureToLock(self):
        ...
        self.displayPositions(x0,y0,percentResize)
        return im
...
#        if np.abs(stepsToMove[0])>self.maxNumberOfSteps:
#            stepsToMove[0] = np.sign(stepsToMove[0])*self.maxNumberOfSteps
#        if np.abs(stepsToMove[1])>self.maxNumberOfSteps:
#            stepsToMove[1] = np.sign(stepsToMove[0])*self.maxNumberOfSteps
    ...
    def displayPositions(self,x0,y0,percentResize):
...
#        self.saveData(data1)
    ...
    def on_targetUpdateButton_clicked(self):
        ...
        self.atomsX0 = self.currentX
        
        """
        try:
            self.targetY = float(self.ui.targetXedit.text())
            self.ui.InformationCamera.append('Target X set to '+str(self.targetY))
            self.atomsY0 = self.targetY
        except:
            print 'problem updating targetX'
        try:    
            self.targetX = float(self.ui.targetYedit.text())
            self.ui.InformationCamera.append('Target Y set to '+str(self.targetX))
            self.atomsX0 = self.targetX
        except:
            print 'problem updating targetY'
        targets = np.asarray([self.targetX,self.targetY])
